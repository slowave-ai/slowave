...
The expanded block below is a historical v4 example and must not be copied as the current contract. The authoritative current block is generated by `slowave setup` from `slowave/cli/setup.py`; Claude Desktop and Cursor users must paste that generated v10 block manually. Updating this long embedded example is tracked separately from the v9 MCP cutover.
...
`slowave_activate(task="<verbatim task>", initial_goal="<short goal>", scope="project:<basename(cwd)>", continuity_id=<optional>)` → store `retrieval_id`, `session_id`, and `continuity_id`. On the first activation of a client conversation omit `continuity_id`; then pass the returned opaque ID unchanged on every later activation in that conversation. Never invent, modify, or reuse it in another conversation.
...
For an event remembered after it happened, clients may also send `occurred_at` as an RFC 3339 timestamp. Slowave sets raw-event `recorded_at` internally; clients never set it. See [temporal memory](temporal_memory.md) for the distinction and examples.

"""End-to-end CLI compatibility regression: full cognitive cycle.

Tests the Slowave core mechanisms, including the separation between semantic
truth/reconsolidation and cue-conditioned retrieval-access feedback, using only:
...
Run (progress output requires -s):
    pytest tests/regression/test_cli_e2e.py -v -s
...
# S1  – contextual-irrelevance target (team retrospective; surfaces on noise queries)
...
class TestE2E:
    """Stateful CLI compatibility regression test.
    ...
    def test_phase4_contextual_irrelevance_preserves_semantic_state(self, cli, db):
        """S1 contextual irrelevance writes access evidence, not truth state."""
        time.time()
        ...
        before = _query(
            db,
            "SELECT is_labile, status, salience, confidence, facets_json FROM schemas WHERE id=?",
            (s1_id,),
        )[0]
        ...
        evidence = _query(
            db,
            "SELECT COALESCE(SUM(irrelevant_count), 0) AS irrelevant "
            "FROM schema_retrieval_evidence WHERE schema_id=? AND pathway='direct'",
            (s1_id,),
        )[0]
        _detail(
            f"S1  irrelevant_access_marks={evidence['irrelevant']}  "
            f"is_labile={after['is_labile']}  status={after['status']}"
        )

        assert irrelevant_marks > 0, "S1 must be shown at least once for explicit feedback"
        assert evidence["irrelevant"] == irrelevant_marks
        assert after == before, "contextual irrelevant feedback must not mutate semantic state"
        assert t1_row["status"] == "active"
        assert t1_row["is_labile"] == 0

        # Explicit useful feedback is a matched access-learning recovery signal;
        # it must not require or alter semantic lability.
        r_recover = cli(
        ...
        _detail(f"S1  semantic state after explicit useful mark: {recovered}")
        assert recovered["is_labile"] == 0

    # ── Phase 4b ──────────────────────────────────────────────────────────────

    def test_phase4b_missing_snapshot_feedback_fails_closed(self, cli, db):
        """Scope-less feedback without a trusted snapshot creates no access evidence."""
        time.time()
        ...
        for i in range(3):
            ...
            _detail(f"untrusted irrelevant mark {i + 1}/3 (no retrieval snapshot)")
        ...
        evidence = _query(
            db,
            "SELECT COUNT(*) AS n FROM schema_retrieval_evidence WHERE schema_id=?",
            (d2_id,),
        )[0]
        _detail(f"D2 after: {after}; access rows={evidence['n']}")

        assert after == before, "untrusted feedback must not mutate semantic state"
        assert evidence["n"] == 0, "missing-snapshot feedback must not create access evidence"

    # ── Phase 5 ───────────────────────────────────────────────────────────────

    def test_phase5_consolidation_hygiene(self, cli, db):
        """Two consolidation passes preserve schema count and semantic hygiene."""
        time.time()
        ...
        # Contextual irrelevance deliberately produces no labile semantic
        # schema. Reconsolidation remains accountable for any independently
        # labile stale/wrong schema but has no required candidate in this flow.
        outcomes = {
            k: recon.get(k, 0)
            for k in ("restabilized", "superseded", "contradicted", "inconclusive")
        }
        _detail(f"reconsolidation outcomes: {outcomes}")
        assert sum(outcomes.values()) == recon.get(
            "examined", 0
        ), f"Outcome counts {outcomes} don't sum to examined={recon.get('examined', 0)}"
    ...
    def test_remembered_update_does_not_change_truth_without_feedback(self, cli, db):
        """A nearby updated claim cannot retire the old claim automatically."""
        time.time()
        ...
        old_id_str = r_old.get("memory_id")  # e.g. "sch_42"
        cli("reinforce", r["retrieval_id"], "--feedback", "missing", "--outcome", "unknown")
        cli("commit", sid, "--outcome", "success")
        ...
        new_id_str = r_new.get("memory_id")
        ...
        # A non-authoritative topical relation may still be recorded.
        rel_rows = _query(
            db,
        ...
        assert old_status and old_status[0]["status"] == "active", (
            "Remember/consolidation must not change semantic truth without explicit feedback; "
            f"got status={old_status[0]['status'] if old_status else 'missing'}"
        )
        if rel_rows:
            assert rel_rows[0]["relation"] == "relates_to"

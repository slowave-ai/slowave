"""CLI regression-test configuration.
...
Run with:
    pytest tests/regression/ -v -p no:randomly
or simply:
    pytest tests/regression/test_cli_e2e.py -v
"""


def pytest_collection_modifyitems(config, items):
    """Re-sort stateful CLI regression tests into definition order."""
    regression = [i for i in items if i.fspath.dirpath().basename == "regression"]
    if len(regression) < 2:
        return
    regression.sort(key=lambda i: (str(i.fspath), i.function.__code__.co_firstlineno))
    non_regression = [i for i in items if i.fspath.dirpath().basename != "regression"]
    items[:] = non_regression + regression
...
_PHASES: dict[str, str] = {
    ...
    "test_remembered_update_does_not_change_truth_without_feedback": "Relations — Client-owned truth authority",
...
def pytest_runtest_logstart(nodeid: str, location) -> None:
    """Print a one-line phase description before each CLI regression test."""
